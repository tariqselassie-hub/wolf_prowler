...
async fn test_large_scale_vector_ops() -> anyhow::Result<()> {
    ...
    tracing::info!("Starting Large-Scale Stress Test: {total_records} records");
    ...
    for i in (0..total_records).step_by(batch_size) {
        ...
        storage
            .insert_batch_records("large_scale".to_string(), records, pk.clone())
            .await?;
        tracing::info!(
            "  Ingested batch {i}-{} in {:?}",
            i + batch_size,
    ...
    tracing::info!("Total Ingestion Time: {:?}", start_total.elapsed());

    // 2. Stress search queries while index is large
    let search_queries = 100;
    let mut search_handles = Vec::new();

    tracing::info!("Starting {search_queries} concurrent searches...");
    ...
    for handle in search_handles {
        handle
            .await
            .map_err(|e| anyhow::anyhow!("Handle join failed: {e}"))?;
    }
    tracing::info!("Concurrent Search Time: {:?}", search_start.elapsed());

    // 3. Test logical deletion at scale
    tracing::info!("Deleting 1,000 records...");
    {
        for i in 0..1000 {
    ...
    // 4. Persistence Test
    tracing::info!("Saving and re-loading database...");
    {
    ...
    let storage_loaded =
        WolfDbStorage::open(path).map_err(|e| anyhow::anyhow!("Failed to re-open storage: {e}"))?;
    let stats = storage_loaded
        .get_info()
        .map_err(|e| anyhow::anyhow!("Failed to get info: {e}"))?;
    tracing::info!("Final Persistence Stats: {stats}");

    // Total should still be 50,000 internal IDs, but 1,000 are deleted
    assert_eq!(
        stats["vector_records"]
            .as_u64()
            .ok_or_else(|| anyhow::anyhow!("Missing vector_records"))?,
        total_records as u64
    );
    assert_eq!(
        stats["vector_deleted"]
            .as_u64()
            .ok_or_else(|| anyhow::anyhow!("Missing vector_deleted"))?,
        1000
    );

    tracing::info!("Large-Scale Stress Test Completed Successfully!");
    Ok(())
}
