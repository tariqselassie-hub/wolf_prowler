...
use wolf_db::storage::{IndexingPolicy, WolfDbStorage};
...
async fn test_selective_indexing_policy() -> anyhow::Result<()> {
    ...
    // 1. Set Policies
    storage
        .set_indexing_policy(table_all.clone(), IndexingPolicy::All)
        .await?;
    storage
        .set_indexing_policy(
            table_selected.clone(),
            IndexingPolicy::Selected(vec!["name".into()]),
        )
        .await?;
    storage
        .set_indexing_policy(table_none.clone(), IndexingPolicy::None)
        .await?;
    ...
    storage
        .insert_record(table_all.clone(), record.clone(), pk.clone())
        .await?;
    storage
        .insert_record(table_selected.clone(), record.clone(), pk.clone())
        .await?;
    storage
        .insert_record(table_none.clone(), record.clone(), pk.clone())
        .await?;
    ...
    assert_eq!(
        res.len(),
        1,
        "Should find record by name in Selected policy"
    );
    ...
    assert_eq!(
        res.len(),
        0,
        "Should NOT find record by role in Selected policy (role not indexed)"
    );

    // CASE C: Table None - Should find by NOTHING
    let res = storage
        .find_by_metadata(table_none.clone(), "name".into(), "Wolf".into(), sk.clone())
        .await?;
    assert_eq!(
        res.len(),
        0,
        "Should NOT find record by name in None policy"
    );
    ...
    storage2
        .insert_record(table_selected.clone(), record2, pk)
        .await?;
    ...
    assert_eq!(
        res.len(),
        0,
        "Policy persistence check: role should still not be indexed"
    );
    ...
    assert_eq!(
        res.len(),
        2,
        "Policy persistence check: name should be indexed (now 2 records)"
    );
