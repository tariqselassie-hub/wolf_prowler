...
use crate::network_protection_simple::BasicNetworkProtection;
...
pub struct SwarmManager {
    ...
    pub             network_protection: Arc<Mutex<BasicNetworkProtection>>,
    /// Wolf Pack State
...
impl SwarmManager {
    ...
    pub fn new(config: SwarmConfig) -> anyhow::Result<Self> {
        ...
        let network_protection = Arc::new(Mutex::new(BasicNetworkProtection::new()));
        ...
        let network_protection_clone = network_protection.clone();
                                   ...
                                   match event {
                                       SwarmEvent::NewListenAddr { address, .. } => {
                                        ...
                                        SwarmEvent::ConnectionEstablished { peer_id, endpoint, .. } => {
                                            // Network protection check
                                            let ip_addr = match &endpoint {
                                                libp2p::core::ConnectedPoint::Listener { send_back_addr, .. } => {
                                                    send_back_addr.iter().find_map(|proto| {
                                                        match proto {
                                            ...
                                            let decision = {
                                                let mut np = network_protection_clone.lock().await;
                                                np.check_incoming_connection(&peer_id, ip_addr).await
                                            };

                                            if !decision.allowed {
                                                warn!("🔥 Network protection blocked connection from {}: {}", peer_id, decision.reason);
                                                if let Some(reporter) = &reputation_reporter {
                                                    reporter.report_event(
                                                        &peer_id.to_string(),
                                                        "Security",
                                                        -0.2,
                                                        format!("Connection blocked: {}", decision.reason)
                                                    ).await;
                                                }
                                                ...
                                                if let Some(sender) = &security_sender {
                                                    let event = SecurityEvent::new(
                                                        ...
                                                        format!("Network protection blocked peer {}", peer_id),
                                            ...
                                            // Firewall check
                                            let firewall_allowed = {
                                                let fw = firewall_clone.read().await;
                                                fw.check_access(
                                            ...
                                            if let Some(reporter) = &reputation_reporter {
                                             ...
                                             {
                                                 let mut np = network_protection_clone.lock().await;
                                                 np.record_connection_success(&peer_id).await;
                                             }
