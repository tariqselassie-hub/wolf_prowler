...
impl PersistenceManager {
    ...
    pub async fn save_peers_batch(&self, peers: &[DbPeer]) -> Result<()> {
        let storage = self.storage.read().await;
        let pk = storage.get_active_pk().context("Database locked")?.to_vec();
        ...
        storage
            .insert_batch_records(TABLE_PEERS.to_string(), records, pk)
            .await?;
        Ok(())
    }
    ...
    pub async fn get_all_active_peers(&self) -> Result<Vec<DbPeer>> {
        ...
        let records = storage
            .get_records_batch(TABLE_PEERS.to_string(), keys, sk.to_vec())
            .await?;
    ...
    pub async fn save_peer_metrics_batch(&self, metrics: &[DbPeerMetrics]) -> Result<()> {
        ...
        for m in metrics {
            ...
            let id = format!("{}_{}", m.peer_id, Utc::now().timestamp_millis());
            records.push(Record {
                id,
                data,
                vector: None,
        ...
        storage
            .insert_batch_records(TABLE_PEER_METRICS.to_string(), records, pk)
            .await?;
        Ok(())
    }
    ...
    pub async fn get_recent_alerts(&self, limit: usize) -> Result<Vec<DbSecurityAlert>> {
        ...
        let records = storage
            .get_records_batch(TABLE_SECURITY_ALERTS.to_string(), keys, sk.to_vec())
            .await?;
    ...
    pub async fn get_recent_logs(&self, limit: usize) -> Result<Vec<DbSystemLog>> {
        ...
        let records = storage
            .get_records_batch(TABLE_SYSTEM_LOGS.to_string(), keys, sk.to_vec())
            .await?;
