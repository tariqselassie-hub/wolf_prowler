...
impl KdfEnum {
    ...
    pub fn derive_key(&self, password: &[u8], salt: &[u8], length: usize) -> Result<Vec<u8>> {
        match self {
            Self::Argon2(kdf) => kdf.derive_key(password, salt, length),
            Self::Pbkdf2(kdf) => kdf.derive_key(password, salt, length),
            Self::Hkdf(kdf) => kdf.derive_key(password, salt, length),
            Self::Scrypt(kdf) => kdf.derive_key(password, salt, length),
        }
    }
    ...
    pub const fn name(&self) -> &'static str {
        match self {
            Self::Argon2(kdf) => kdf.name(),
            Self::Pbkdf2(kdf) => kdf.name(),
            Self::Hkdf(kdf) => kdf.name(),
            Self::Scrypt(kdf) => kdf.name(),
        }
    }
    ...
    pub const fn is_memory_hard(&self) -> bool {
        match self {
            Self::Argon2(kdf) => kdf.is_memory_hard(),
            Self::Pbkdf2(kdf) => kdf.is_memory_hard(),
            Self::Hkdf(kdf) => kdf.is_memory_hard(),
            Self::Scrypt(kdf) => kdf.is_memory_hard(),
        }
    }
    ...
    pub fn derivation_count(&self) -> u64 {
        match self {
            Self::Argon2(kdf) => kdf.derivation_count(),
            Self::Pbkdf2(kdf) => kdf.derivation_count(),
            Self::Hkdf(kdf) => kdf.derivation_count(),
            Self::Scrypt(kdf) => kdf.derivation_count(),
        }
    }
    ...
    /// # Errors
    ///
    /// Returns an error if KDF creation fails.
    pub fn new_kdf(&self) -> Result<Self> {
        // The new_hasher method on HasherEnum now correctly creates a new instance of the same type.
        match self {
            Self::Argon2(kdf) => Ok(Self::Argon2(Argon2Kdf::new(kdf.security_level)?)),
            Self::Pbkdf2(kdf) => Ok(Self::Pbkdf2(Pbkdf2Kdf::new(
                kdf.security_level,
                kdf.hasher.new_hasher()?,
            )?)),
            Self::Hkdf(kdf) => Ok(Self::Hkdf(HkdfKdf::new(
                kdf.security_level,
                kdf.hasher.new_hasher()?,
            )?)),
            Self::Scrypt(kdf) => Ok(Self::Scrypt(ScryptKdf::new(kdf.security_level)?)),
        }
    }
}
...
impl Argon2Kdf {
    ...
    /// * `Maximum` - `262_144` memory, 4 passes, 4 parallelism, 32-bit output
    ///
    /// # Errors
    ///
    ...
    pub fn new(security_level: crate::SecurityLevel) -> Result<Self> {
        let params = match security_level {
            ...
            crate::SecurityLevel::Maximum => argon2::Params::new(262_144, 4, 4, Some(32))?,
        };
    ...
    /// # Errors
    ///
    /// Returns an error if Argon2 derivation fails.
    pub fn derive_key(&self, password: &[u8], salt: &[u8], length: usize) -> Result<Vec<u8>> {
...
impl Pbkdf2Kdf {
    ...
    ///
    /// Returns an error if initialization fails.
    pub const fn new(security_level: crate::SecurityLevel, hasher: HasherEnum) -> Result<Self> {
...
impl HkdfKdf {
    ...
    /// # Errors
    ///
    /// Returns an error if HKDF derivation fails.
    pub fn derive_key(&self, password: &[u8], salt: &[u8], length: usize) -> Result<Vec<u8>> {
        use hkdf::Hkdf;
...
impl ScryptKdf {
    /// Create a new `ScryptKdf` instance
    ///
    /// # Errors
    ///
    /// Returns an error if initialization fails.
    ...
    /// # Errors
    ///
    /// Returns an error if Scrypt derivation fails.
    pub fn derive_key(&self, password: &[u8], salt: &[u8], length: usize) -> Result<Vec<u8>> {
        let mut key = vec![0u8; length];
...
mod tests {
    ...
    fn test_argon2_kdf() {
        ...
        let key = kdf.derive_key(password, salt, length).unwrap();
    ...
    #[test]
    #[allow(clippy::unwrap_used)]
    fn test_key_derivation_wrapper() {
    ...
    fn test_kdf_consistency() {
        ...
        let key1 = kdf.derive_key(password, salt, length).unwrap();
        let key2 = kdf.derive_key(password, salt, length).unwrap();
    ...
    fn test_kdf_different_salts() {
        ...
        let key1 = kdf.derive_key(password, salt1, length).unwrap();
        let key2 = kdf.derive_key(password, salt2, length).unwrap();
    ...
    fn test_kdf_different_passwords() {
        ...
        let key1 = kdf.derive_key(password1, salt, length).unwrap();
        let key2 = kdf.derive_key(password2, salt, length).unwrap();
    ...
    async fn test_kdf_derivation_count() {
        ...
        assert_eq!(kdf.derivation_count(), 0);

        let _key = kdf.derive_key(password, salt, length).unwrap();
        assert_eq!(kdf.derivation_count(), 1);

        let _key = kdf.derive_key(password, salt, length).unwrap();
        assert_eq!(kdf.derivation_count(), 2);
    }
    ...
    fn test_different_kdf_types() {
        ...
        let argon2_key = argon2.derive_key(password, salt, length).unwrap();
        let pbkdf2_key = pbkdf2.derive_key(password, salt, length).unwrap();
        let hkdf_key = hkdf.derive_key(password, salt, length).unwrap();
        let scrypt_key = scrypt.derive_key(password, salt, length).unwrap();
