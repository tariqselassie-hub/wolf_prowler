...
impl AuthManager {
    ...
    pub async fn create_user(
        ...
        let password_hash = self.hash_password(password.as_bytes()).await?;
        let user_id = Uuid::new_v4();

        // Convert auth::User to domain::entities::User if needed, or vice-versa
        let mut roles = HashSet::new();
        roles.insert(format!("{:?}", role).to_lowercase());

        let domain_user = crate::domain::entities::auth::User {
            id: user_id,
            username: username.to_string(),
            password_hash: password_hash.0.clone(),
            roles,
            is_active: true,
            created_at: Utc::now(),
            updated_at: Utc::now(),
        ...
        self.repository
            .save_user(&domain_user)
            .await
            .map_err(|e| anyhow!("Failed to save user: {}", e))?;
        Ok(user_id)
    }

    /// Authenticates a user and returns a session-like object.
    pub async fn authenticate(&self, username: &str, password: &str) -> Result<AuthSession> {
        let user_opt = self
            .repository
            .find_user_by_username(username)
            .await
            .map_err(|e| anyhow!("Repository error: {}", e))?;
        let user = user_opt.ok_or_else(|| anyhow!("User not found"))?;

        let is_valid = self
            .verify_password(password.as_bytes(), &PasswordHashString(user.password_hash))
            .await?;
        if !is_valid {
...
/// Simple session object for AuthManager
