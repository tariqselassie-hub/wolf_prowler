...
async fn test_rbac_authorization() -> Result<()> {
    ...
    assert!(decision.decision == crate::identity::iam::AccessDecisionType::Allow);
    ...
    let decision = rbac_manager.check_access(query).await?;
    assert!(decision.decision == crate::identity::iam::AccessDecisionType::Deny);
...
async fn test_session_management() -> Result<()> {
    ...
    assert_eq!(
        session.status,
        crate::identity::iam::SessionStatus::Active
    );
    ...
    let update_request = crate::identity::iam::session::SessionUpdateRequest {
        session_id: session.id,
        client_info: Some(Default::default()),
        activity_type: crate::identity::iam::session::SessionActivityType::APIRequest,
        context: None,
    };
...
async fn test_comprehensive_auth_flow() -> Result<()> {
    ...
    // Create test user
    let user_request = crate::identity::iam::user_management::CreateUserRequest {
        username: "test_user".to_string(),
    ...
    // Test authentication
    let auth_request = crate::identity::iam::authentication::AuthenticationRequest {
        username: "test_user".to_string(),
    ...
    // Test authorization
    let authz_request = crate::identity::iam::authorization::AuthorizationRequest {
        user_id: user.id,
    ...
    assert!(decision.decision == crate::identity::iam::Effect::Allow);
...
async fn test_rbac_role_hierarchy() -> Result<()> {
    ...
    assert!(alpha_decision.decision == crate::identity::iam::AccessDecisionType::Allow);
    ...
    assert!(omega_decision.decision == crate::identity::iam::AccessDecisionType::Allow);
    ...
    assert!(
        omega_admin_decision.decision == crate::identity::iam::AccessDecisionType::Deny
    );
...
async fn test_session_security() -> Result<()> {
    ...
    let malicious_request = crate::identity::iam::session::SessionUpdateRequest {
        session_id: session.id,
        client_info: Some(crate::identity::iam::ClientInfo {
    ...
    let user_agent_request = crate::identity::iam::session::SessionUpdateRequest {
        session_id: session.id,
        client_info: Some(crate::identity::iam::ClientInfo {
