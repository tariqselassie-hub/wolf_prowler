...
impl MonitoringRepository for WolfDbMonitoringRepository {
    async fn save_event(&self, event: &SecurityEvent) -> Result<(), DomainError> {
        ...
        let json_str =
            serde_json::to_string(event).map_err(|e| DomainError::Unexpected(e.to_string()))?;
        ...
        storage
            .insert_record(TABLE_SECURITY_EVENTS.to_string(), record, pk)
            .await
    ...
    async fn find_event_by_id(&self, id: &Uuid) -> Result<Option<SecurityEvent>, DomainError> {
        ...
        if let Some(record) = storage
            .get_record(
                TABLE_SECURITY_EVENTS.to_string(),
                id.to_string(),
                sk.to_vec(),
            )
            .await
    ...
    async fn get_recent_events(&self, limit: usize) -> Result<Vec<SecurityEvent>, DomainError> {
        ...
        let keys = storage
            .list_keys(TABLE_SECURITY_EVENTS.to_string())
            .await
            .map_err(|e| DomainError::Unexpected(e.to_string()))?;

        let records = storage
            .get_records_batch(TABLE_SECURITY_EVENTS.to_string(), keys, sk.to_vec())
            .await
            .map_err(|e| DomainError::Unexpected(e.to_string()))?;
