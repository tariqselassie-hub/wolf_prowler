...
impl AuthRepository for WolfDbAuthRepository {
    async fn save_user(&self, user: &User) -> Result<(), DomainError> {
        ...
        let json_str =
            serde_json::to_string(user).map_err(|e| DomainError::Unexpected(e.to_string()))?;
        ...
        storage
            .insert_record(TABLE_USERS.to_string(), record, pk)
            .await
    ...
    async fn find_user_by_id(&self, id: &Uuid) -> Result<Option<User>, DomainError> {
        ...
        if let Some(record) = storage
            .get_record(TABLE_USERS.to_string(), id.to_string(), sk.to_vec())
            .await
            .map_err(|e| DomainError::Unexpected(e.to_string()))?
        {
    ...
    async fn save_role(&self, role: &Role) -> Result<(), DomainError> {
        ...
        let json_str =
            serde_json::to_string(role).map_err(|e| DomainError::Unexpected(e.to_string()))?;
        ...
        storage
            .insert_record(TABLE_ROLES.to_string(), record, pk)
            .await
            .map_err(|e| DomainError::Unexpected(e.to_string()))?;
