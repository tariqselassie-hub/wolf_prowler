...
pub struct SecurityMonitor {
    ...
    monitoring_repo: Arc<dyn crate::domain::repositories::MonitoringRepository>,
    /// repository for persisting security alerts and resolutions.
    alert_repo: Arc<dyn crate::domain::repositories::AlertRepository>,
}
...
impl SecurityMonitor {
    ...
    /// Get all security events from memory cache
    pub async fn get_events(&self) -> Vec<SecurityEvent> {
        let siem = self.siem.read().await;
        siem.events.clone()
    }

    /// Get recent security events from persistent storage (optimized parallel pull)
    pub async fn get_recent_persisted_events(
        &self,
        limit: usize,
    ) -> Result<Vec<crate::domain::entities::monitoring::SecurityEvent>> {
        self.monitoring_repo
            .get_recent_events(limit)
            .await
            .map_err(|e| anyhow!("Failed to fetch persisted events: {}", e))
    }
