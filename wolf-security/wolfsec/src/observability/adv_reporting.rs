...
impl SecurityReporter {
    pub fn new(config: ReportingConfig) -> Result<Self, Error> {
        Ok(Self {
            config,
...
impl SecurityReportGenerator {
    ...
    pub async fn generate_report(&self, time_range: TimeRange) -> Result<SecurityReport, Error> {
        ...
        let alerts = self.collect_alerts(&time_range)?;
        let audit_data = self.collect_audit_data(&time_range)?;
        let compliance = self.collect_compliance_data(&time_range)?;
        ...
        let recommendations =
            self.generate_recommendations(&summary, &security_status, &alerts, &compliance)?;
        ...
        let report = SecurityReport {
            ...
            compliance,
            recommendations,
            metadata: self.generate_report_metadata(&time_range)?,
        };
    ...
    pub async fn export_report(
        ...
        match format {
            ...
            ReportFormat::Xml => self.export_xml(&report),
    ...
    pub async fn start_automated_generation(&self) -> Result<(), Error> {
        ...
        *is_generating = true;
        ...
        tokio::spawn(async move {
            loop {
                if !*is_generating.read().await {
                    break;
                }

                // Generate daily report
                if let Err(e) = Self::generate_daily_report(&reports, &config) {
                    error!("Failed to generate daily report: {}", e);
                }

                // Wait for next generation
                let sleep_duration =
                    tokio::time::Duration::from_secs(config.report_interval_hours as u64 * 3600);
    ...
    async fn collect_security_status(
        ...
        Ok(SecurityStatus {
            ...
            last_update: Utc::now(),
            timestamp: Utc::now(),
            metrics: crate::observability::metrics::SecurityMetrics::default(),
            audit_summary: crate::observability::audit::AuditSummary::default(),
        })
    }
    ...
    /// Collect alerts
    fn collect_alerts(&self, time_range: &TimeRange) -> Result<ReportAlerts, Error> {
    ...
    /// Collect audit data
    fn collect_audit_data(&self, _time_range: &TimeRange) -> Result<ReportAudit, Error> {
    ...
    fn collect_compliance_data(&self, time_range: &TimeRange) -> Result<ReportCompliance, Error> {
    ...
    fn generate_summary(
        ...
        let overall_security_score = (*security_status
            .metrics
            .get("security_score")
            .unwrap_or(&0.0)
    ...
    fn export_json(&self, report: &SecurityReport) -> Result<Vec<u8>, Error> {
        let json = serde_json::to_string_pretty(report)?;
        Ok(json.into_bytes())
    }

    /// Export report as HTML
    fn export_html(&self, report: &SecurityReport) -> Result<Vec<u8>, Error> {
        let html = format!(
            r#"
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    fn export_csv(&self, report: &SecurityReport) -> Result<Vec<u8>, Error> {
    ...
    fn export_xml(&self, report: &SecurityReport) -> Result<Vec<u8>, Error> {
        let xml = format!(
            r#"
...
<security_report>
    ...
    fn export_pdf(&self, _report: &SecurityReport) -> Result<Vec<u8>, Error> {
