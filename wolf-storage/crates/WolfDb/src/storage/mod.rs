                    ...
                    for (k, v) in meta_tree.iter().flatten() {
                        ...
                        if let Some(table_name) = key_str.strip_prefix("policy:") {
                            if let Ok(policy) = serde_json::from_slice::<IndexingPolicy>(&v) {
                                indexing_policies
                                    .write()
                                    .map_err(|_| WolfDbError::LockPoisoned)?
...
impl Drop for WolfDbStorage {
    fn drop(&mut self) {
        // Attempt to save vector indices on exit to prevent data loss
        if let Err(e) = self.save() {
            tracing::error!("Failed to auto-save vector indices during shutdown: {e}");
        } else {
            tracing::debug!("Successfully auto-saved vector indices.");
        }
    }
}
...
impl WolfDbStorage {
    ...
    pub fn unlock(&mut self, password: &str, hsm_pin: Option<&str>) -> Result<()> {
        ...
        if let Some(dsa_sk_unlocked) = maybe_dsa_sk_unlocked {
            ...
            let kem_secret_bytes = self
                .kem_secret_key
                .as_ref()
                .ok_or_else(|| WolfDbError::Crypto("KEM SK missing during migration".to_string()))?
                .as_slice();
            let kem_public_bytes = self
                .kem_public_key
                .as_ref()
                .ok_or_else(|| WolfDbError::Crypto("KEM PK missing during migration".to_string()))?;
            ...
            let dsa_sk_zeroized = self
                .dsa_secret_key
                .as_ref()
                .ok_or_else(|| WolfDbError::Crypto("DSA SK missing after migration".to_string()))?;
            let dsa_pk_ref = self
                .dsa_public_key
                .as_ref()
                .ok_or_else(|| WolfDbError::Crypto("DSA PK missing after migration".to_string()))?;
            let keypair = crate::crypto::signature::reconstruct_keypair(
    ...
    /// Retrieves and decrypts multiple records by their IDs in parallel
    ///
    /// # Errors
    ///
    /// Returns an error if any record retrieval or decryption fails.
    pub async fn get_records_batch(
        ...
        let crypto_manager = self.crypto;
        let dsa_public_key = self.dsa_public_key.clone();
        let sk_arc = Arc::new(kem_sk);

        let mut set = tokio::task::JoinSet::new();

        for id in ids {
            let engine = storage_engine.clone();
            let table_name = clean_table.clone();
            let sk_task = sk_arc.clone();
            let dsa_pk = dsa_public_key.clone();

            set.spawn_blocking(move || -> Result<Option<model::Record>> {
                let bin = engine
                    .get(&table_name, id.as_bytes())
                    .map_err(|e| WolfDbError::Storage(e.to_string()))?;
                if let Some(encrypted_bin) = bin {
                    let encrypted: EncryptedData = bincode::deserialize(&encrypted_bin)?;
                    let dsa_pk_ref = dsa_pk.as_deref();
                    let decrypted = crypto_manager
                        .decrypt_at_rest(&encrypted, &sk_task, dsa_pk_ref)
                        .map_err(|e| WolfDbError::Crypto(e.to_string()))?;
                    let record: model::Record = bincode::deserialize(&decrypted)?;
                    Ok(Some(record))
                } else {
    ...
    pub async fn find_by_metadata(
        ...
        // Second pass: fetch actual records in parallel
        self.get_records_batch(table, ids, kem_sk).await
    }
    ...
    pub async fn search_hybrid(
        ...
        // 3. Fetch Records in parallel
        let ids: Vec<String> = scores.iter().map(|(id, _)| id.clone()).collect();
        let records = self.get_records_batch(table, ids, kem_sk).await?;

        // Re-combine records with their scores
        let mut final_results = Vec::new();
        for (id, score) in scores {
            if let Some(record) = records.iter().find(|r| r.id == id) {
                final_results.push((record.clone(), score));
            }
        }
        Ok(final_results)
    }
    ...
    pub async fn search_similar_records(
        ...
        let ids: Vec<String> = scores.iter().map(|(id, _)| id.clone()).collect();
        let records = self.get_records_batch(table.clone(), ids, kem_sk).await?;
    ...
    /// Rebuilds a vector index by scanning and decrypting all records in a table
    ///
    /// # Errors
    ///
    /// Returns an error if record scanning or decryption fails.
    pub async fn rebuild_vector_index(&self, table: String, kem_sk: Vec<u8>) -> Result<()> {
        let (partition, clean_table) = Partition::from_table(&table);
        if !partition.supports_vectors() {
            return Err(WolfDbError::InvalidPartition(
                "No vector support".to_string(),
        ...
        let sk_clone = kem_sk.clone();
        let storage_engine = engine;
        let crypto_manager = self.crypto;
        let dsa_pk = self.dsa_public_key.clone();
        ...
        tokio::task::spawn_blocking(move || -> Result<()> {
            ...
            for key_bytes in keys {
                let bin = storage_engine
                    .get(&clean_table, &key_bytes)
                    .map_err(|e| WolfDbError::Storage(e.to_string()))?
                    .ok_or_else(|| WolfDbError::Storage("Key vanished during scan".to_string()))?;

                let encrypted: EncryptedData = bincode::deserialize(&bin)?;
                let decrypted = crypto_manager
                    .decrypt_at_rest(&encrypted, &sk_clone, dsa_pk.as_deref())
                    .map_err(|e| WolfDbError::Crypto(e.to_string()))?;
                let record: model::Record = bincode::deserialize(&decrypted)?;

                if let Some(vec) = record.vector {
                    batch.push((record.id, vec));
                }

                if batch.len() >= 1000 {
                    vector_index
                        .insert_batch(batch.drain(..).collect())
                        .map_err(|e| WolfDbError::Vector(e.to_string()))?;
                }
            }
    ...
    pub async fn set_indexing_policy(&self, table: String, policy: IndexingPolicy) -> Result<()> {
        ...
        {
            let mut cache = self.indexing_policies.write().map_err(|_| WolfDbError::LockPoisoned)?;
    ...
    pub fn generate_recovery_backup(&self, recovery_password: &str) -> Result<String> {
        ...
        let kem_public_key_raw = self
            .kem_public_key
            .as_ref()
            .ok_or_else(|| WolfDbError::Crypto("KEM PK missing during backup".to_string()))?;
        let dsa_public_key_raw = self
            .dsa_public_key
            .as_ref()
            .ok_or_else(|| WolfDbError::Crypto("DSA PK missing during backup".to_string()))?;
