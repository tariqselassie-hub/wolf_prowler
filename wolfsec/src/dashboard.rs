...
impl SecurityDashboard {
    ...
    #[instrument(skip(self))]
    pub async fn update_dashboard_data(&self, new_data: SecurityDashboardData) -> Result<(), Error> {
        let mut data = self.data.write().await;
        *data = new_data;
        Ok(())
    }
    ...
    pub async fn add_alert(&self, alert: super::alerts::SecurityAlert) -> Result<(), Error> {
        ...
        if let Some(WidgetData::Alerts(alerts_widget)) = data.widgets.get_mut("alerts") {
            alerts_widget.total_alerts = data.alerts.len();
            alerts_widget.critical_alerts = data.alerts.iter()
                .filter(|a| a.severity == super::alerts::AlertSeverity::Critical)
                .count();
            alerts_widget.high_alerts = data.alerts.iter()
                .filter(|a| a.severity == super::alerts::AlertSeverity::High)
                .count();
            alerts_widget.medium_alerts = data.alerts.iter()
                .filter(|a| a.severity == super::alerts::AlertSeverity::Medium)
                .count();
            alerts_widget.low_alerts = data.alerts.iter()
                .filter(|a| a.severity == super::alerts::AlertSeverity::Low)
                .count();
            alerts_widget.recent_alerts = data.alerts.iter().take(10).cloned().collect();
    ...
    #[instrument(skip(self))]
    pub async fn update_performance_metrics(&self, metrics: PerformanceMetrics) -> Result<(), Error> {
        let mut data = self.data.write().await;
        data.performance_metrics = metrics.clone();

        // Update performance widget
        if let Some(WidgetData::Performance(performance_widget)) = data.widgets.get_mut("performance") {
    ...
    #[instrument(skip(self))]
    pub async fn update_network_security_metrics(&self, metrics: NetworkSecurityMetrics) -> Result<(), Error> {
        let mut data = self.data.write().await;
        data.network_security = metrics.clone();

        // Update network security widget
        if let Some(WidgetData::NetworkSecurity(network_widget)) = data.widgets.get_mut("network_security") {
        ...
        if self.config.enable_real_time_updates {
            ...
            tokio::spawn(async move {
                while *is_running.read().await {
                    ...
                    tokio::time::sleep(tokio::time::Duration::from_secs(config.refresh_interval_secs)).await;
                }
            });
        }
    ...
    async fn refresh_dashboard_data(
        ...
        // Update performance widget
        if let Some(WidgetData::Performance(performance_widget)) = dashboard_data.widgets.get_mut("performance") {
            performance_widget.cpu_usage = dashboard_data.performance_metrics.cpu_usage_percent;
            performance_widget.memory_usage = dashboard_data.performance_metrics.memory_usage_percent;
            performance_widget.last_updated = Utc::now();
        }
...
<body>
    ...
    <script>
        ...
        "#,
            ...
            data.security_status.metrics.security_score,
            data.security_status.metrics.operation_success_rate * 100.0,
            data.security_status.metrics.anomaly_score,
            data.alerts.len(),
            data.alerts.iter().filter(|a| a.severity == super::alerts::AlertSeverity::Critical).count(),
            data.alerts.iter().filter(|a| a.severity == super::alerts::AlertSeverity::High).count(),
...
mod tests {
    ...
    async fn test_dashboard_data_management() {
        let dashboard = SecurityDashboard::new(SecurityDashboardConfig::default()).await.unwrap();
        ...
        // Test adding alert
        let alert = super::alerts::SecurityAlert {
            id: "test-alert".to_string(),
            timestamp: Utc::now(),
            severity: super::alerts::AlertSeverity::Medium,
