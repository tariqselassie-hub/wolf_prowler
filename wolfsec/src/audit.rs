...
impl SecurityAuditor {
    ...
    pub async fn record_operation(
        ...
        debug!("Audit entry recorded: {} - {}", entry_id, operation.as_str());
        Ok(entry_id)
    }
    ...
    pub async fn record_detailed_operation(
        ...
        debug!("Detailed audit entry recorded: {} - {}", entry_id, operation.as_str());
        Ok(entry_id)
    }
    ...
    pub async fn get_entries_by_operation(&self, operation: SecurityOperation) -> Vec<AuditEntry> {
        let entries = self.audit_entries.read().await;
        entries.iter().filter(|e| e.operation == operation).cloned().collect()
    }
    ...
    pub async fn get_entries_by_category(&self, category: SecurityCategory) -> Vec<AuditEntry> {
        let entries = self.audit_entries.read().await;
        entries.iter().filter(|e| e.operation.category() == category).cloned().collect()
    }
    ...
    #[instrument(skip(self))]
    pub async fn get_entries_in_time_range(&self, start: DateTime<Utc>, end: DateTime<Utc>) -> Vec<AuditEntry> {
        let entries = self.audit_entries.read().await;
        entries
            .iter()
    ...
    pub async fn get_entries_by_risk_level(&self, risk_level: RiskLevel) -> Vec<AuditEntry> {
        let entries = self.audit_entries.read().await;
        entries.iter().filter(|e| e.risk_level == risk_level).cloned().collect()
    }
    ...
    pub async fn get_audit_summary(&self) -> AuditSummary {
        ...
        let successful_operations = entries.iter().filter(|e| e.result.is_successful()).count() as u64;
        ...
        let high_risk_operations = entries.iter().filter(|e| e.risk_level == RiskLevel::High).count() as u64;
        let critical_risk_operations = entries.iter().filter(|e| e.risk_level == RiskLevel::Critical).count() as u64;
    ...
    fn calculate_risk_level(&self, operation: &SecurityOperation, result: OperationResult) -> RiskLevel {
    ...
    fn calculate_compliance_summary(&self, entries: &[AuditEntry]) -> ComplianceSummary {
        ...
        for entry in entries {
            ...
            for tag in &entry.compliance_tags {
                let status = compliance_by_standard.entry(tag.clone()).or_insert(ComplianceStatus {
                    standard: tag.clone(),
...
mod tests {
    ...
    async fn test_operation_recording() {
        let auditor = SecurityAuditor::new(SecurityAuditConfig::default()).await.unwrap();
    ...
    async fn test_detailed_operation_recording() {
        ...
        let entry_id = auditor.record_detailed_operation(
            SecurityOperation::KeyGeneration,
            OperationResult::Success,
            "system".to_string(),
    ...
    async fn test_audit_summary() {
        ...
        for i in 0..5 {
            auditor.record_operation(
                if i < 2 { SecurityOperation::Encryption } else { SecurityOperation::Decryption },
                OperationResult::Success,
                "test_user".to_string(),
                format!("Operation {}", i),
            ).await.unwrap();
    ...
    async fn test_audit_filtering() {
        ...
        let encryption_entries = auditor.get_entries_by_operation(SecurityOperation::Encryption).await;
        assert_eq!(encryption_entries.len(), 1);
        
        // Filter by category
        let crypto_entries = auditor.get_entries_by_category(SecurityCategory::Cryptographic).await;
