...
use anyhow::Error;
use super::{TimeRange, SecurityStatus, SecurityStatusLevel};
...
impl ReportType {
    ...
    pub fn default_time_range(&self) -> TimeRange {
        ...
        match self {
            ReportType::Daily => TimeRange { start: now.date_naive().and_hms_opt(0, 0, 0).unwrap().and_utc(), end: now },
            ReportType::Weekly => TimeRange { start: now - chrono::Duration::days(7), end: now },
            ReportType::Monthly => TimeRange { start: now - chrono::Duration::days(30), end: now },
            ReportType::Quarterly => TimeRange { start: now - chrono::Duration::days(90), end: now },
            ReportType::Yearly => TimeRange { start: now - chrono::Duration::days(365), end: now },
            ReportType::Custom => TimeRange { start: now - chrono::Duration::hours(24), end: now },
            ReportType::Incident => TimeRange { start: now - chrono::Duration::hours(24), end: now },
            ReportType::Compliance => TimeRange { start: now - chrono::Duration::days(30), end: now },
            ReportType::Security => TimeRange { start: now - chrono::Duration::hours(24), end: now },
            ReportType::Performance => TimeRange { start: now - chrono::Duration::hours(24), end: now },
        }
    }
}
...
impl SecurityReportGenerator {
    ...
    pub async fn generate_report(&self, time_range: TimeRange) -> Result<SecurityReport, Error> {
        ...
        info!("Generating security report: {} for range {:?}", report_id, time_range);
        ...
        // Generate summary
        let summary = self.generate_summary(&security_status, &metrics, &alerts, &audit_data, &compliance).await?;
        
        // Generate recommendations
        let recommendations = self.generate_recommendations(&summary, &security_status, &alerts, &compliance).await?;
    ...
    #[instrument(skip(self))]
    pub async fn export_report(&self, report_id: &str, format: ReportFormat) -> Result<Vec<u8>, Error> {
        let report = self.get_report(report_id).await.ok_or_else(|| {
        ...
        *is_generating = true;
        ...
        tokio::spawn(async move {
            while *is_generating.read().await {
                ...
                tokio::time::sleep(tokio::time::Duration::from_secs(config.report_interval_hours as u64 * 3600)).await;
            }
        });
    ...
    fn generate_report_title(&self, report_type: &ReportType, time_range: &TimeRange) -> String {
        match report_type {
            ReportType::Daily => format!("Daily Security Report - {}", time_range.start.format("%Y-%m-%d")),
            ReportType::Weekly => format!("Weekly Security Report - {} to {}", 
                time_range.start.format("%Y-%m-%d"), time_range.end.format("%Y-%m-%d")),
            ReportType::Monthly => format!("Monthly Security Report - {}", time_range.start.format("%Y-%m")),
            ReportType::Quarterly => format!("Quarterly Security Report - Q{} {}", 
                time_range.start.quarter(), time_range.start.year()),
            ReportType::Yearly => format!("Yearly Security Report - {}", time_range.start.year()),
            ReportType::Custom => format!("Custom Security Report - {} to {}", 
                time_range.start.format("%Y-%m-%d"), time_range.end.format("%Y-%m-%d")),
            ReportType::Incident => format!("Security Incident Report - {}", time_range.start.format("%Y-%m-%d")),
            ReportType::Compliance => format!("Compliance Report - {}", time_range.start.format("%Y-%m-%d")),
            ReportType::Security => format!("Security Assessment Report - {}", time_range.start.format("%Y-%m-%d")),
            ReportType::Performance => format!("Performance Security Report - {}", time_range.start.format("%Y-%m-%d")),
        }
    }

    /// Collect security status
    async fn collect_security_status(&self, time_range: &TimeRange) -> Result<SecurityStatus, Error> {
    ...
    async fn collect_compliance_data(&self, time_range: &TimeRange) -> Result<ReportCompliance, Error> {
        // This would normally collect real compliance data
        // For now, we'll simulate some data
        let mut compliance_by_standard = HashMap::new();
        compliance_by_standard.insert("SOC2".to_string(), ComplianceScore {
            standard: "SOC2".to_string(),
            score: 95.0,
            status: ComplianceStatus::Compliant,
            last_assessment: time_range.end,
        });
        compliance_by_standard.insert("GDPR".to_string(), ComplianceScore {
    ...
    async fn generate_summary(
        ...
        let overall_security_score = (security_status.metrics.security_score + 
            metrics.performance.availability_percent + 
            compliance.overall_compliance_score) / 3.0;

        let total_events = alerts.total_alerts + audit_data.total_entries;
        let critical_events = alerts.alerts_by_severity.get("critical").unwrap_or(&0) + audit_data.critical_operations;
        let high_severity_events = alerts.alerts_by_severity.get("high").unwrap_or(&0) + audit_data.high_risk_operations;
        ...
        let key_findings = vec![
            ...
            format!("Compliance score: {:.1}%", compliance.overall_compliance_score),
            format!("System availability: {:.1}%", metrics.performance.availability_percent),
        ];
    ...
    async fn generate_recommendations(
        ...
        if summary.overall_security_score < 80.0 {
            recommendations.push(Recommendation {
                ...
                description: "Implement additional security controls to improve the overall security score".to_string(),
        ...
        if compliance.overall_compliance_score < 90.0 {
            recommendations.push(Recommendation {
                ...
                description: "Remediate compliance violations to improve compliance score".to_string(),
                priority: RecommendationPriority::Medium,
        ...
        if alerts.total_alerts > 100 {
            recommendations.push(Recommendation {
                ...
                description: "Review and optimize alert thresholds to reduce alert fatigue".to_string(),
    ...
    async fn generate_report_metadata(&self, time_range: &TimeRange) -> Result<HashMap<String, String>, Error> {
        ...
        metadata.insert("time_range_start".to_string(), time_range.start.to_rfc3339());
...
<body>
    ...
    <div class="section">
        ...
        "#,
            ...
            report.alerts.alerts_by_severity.get("critical").unwrap_or(&0),
            ...
            report.recommendations.iter().map(|r| format!(
    ...
    async fn export_csv(&self, report: &SecurityReport) -> Result<Vec<u8>, Error> {
        ...
        csv.push_str(&format!("Overall Security Score,{:.1}%\n", report.summary.overall_security_score));
        ...
        csv.push_str(&format!("Compliance Score,{:.1}%\n", report.compliance.overall_compliance_score));
        csv.push_str(&format!("Availability,{:.1}%\n", report.metrics.performance.availability_percent));
        Ok(csv.into_bytes())
    }
...
<security_report>
    ...
    async fn export_pdf(&self, report: &SecurityReport) -> Result<Vec<u8>, Error> {
        ...
        let font = doc.add_builtin_font(BuiltinFont::Helvetica).map_err(|e| anyhow::anyhow!(e))?;
        let font_bold = doc.add_builtin_font(BuiltinFont::HelveticaBold).map_err(|e| anyhow::anyhow!(e))?;
        
        // Function to draw text
        let draw_text = |layer: &PdfLayerReference, font: &IndirectFontRef, text: &str, x: f64, y: f64, size: f64| {
            layer.use_text(text, size, Mm(x), Mm(y), font);
        };

        // Header
        draw_text(&current_layer, &font_bold, &report.title, 20.0, 280.0, 24.0);
        draw_text(&current_layer, &font, &format!("Generated: {}", report.generated_at), 20.0, 270.0, 12.0);
        draw_text(&current_layer, &font, &format!("ID: {}", report.id), 20.0, 265.0, 10.0);
        
        // Summary
        draw_text(&current_layer, &font_bold, "Executive Summary", 20.0, 250.0, 16.0);
        
        // Wrap executive summary text (simple wrapping)
        let words: Vec<&str> = report.summary.executive_summary.split_whitespace().collect();
        ...
        draw_text(&current_layer, &font_bold, "Key Metrics", 20.0, y_metrics, 14.0);
        
        draw_text(&current_layer, &font, &format!("Overall Score: {:.1}%", report.summary.overall_security_score), 20.0, y_metrics - 10.0, 12.0);
        draw_text(&current_layer, &font, &format!("Total Events: {}", report.summary.total_events), 20.0, y_metrics - 15.0, 12.0);
        draw_text(&current_layer, &font, &format!("Critical Events: {}", report.summary.critical_events), 20.0, y_metrics - 20.0, 12.0);
        draw_text(&current_layer, &font, &format!("Compliance Score: {:.1}%", report.compliance.overall_compliance_score), 100.0, y_metrics - 10.0, 12.0);
        draw_text(&current_layer, &font, &format!("Availability: {:.1}%", report.metrics.performance.availability_percent), 100.0, y_metrics - 15.0, 12.0);
...
mod tests {
    ...
    async fn test_report_generation() {
        let generator = SecurityReportGenerator::new(ReportingConfig::default()).await.unwrap();
        
        let time_range = TimeRange::last_hours(24);
        let report = generator.generate_report(time_range).await;
        ...
        assert!(report.summary.overall_security_score >= 0.0 && report.summary.overall_security_score <= 100.0);
    }
    ...
    async fn test_report_export() {
        ...
        let time_range = TimeRange::last_hours(24);
        let report = generator.generate_report(time_range).await.unwrap();
        
        // Test JSON export
        let json_data = generator.export_report(&report.id, ReportFormat::Json).await;
        assert!(json_data.is_ok());
        
        // Test HTML export
        let html_data = generator.export_report(&report.id, ReportFormat::Html).await;
