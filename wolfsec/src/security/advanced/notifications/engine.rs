...
impl NotificationEngine {
    ...
    pub async fn send_notification(&self, request: NotificationRequest) -> Vec<NotificationResult> {
        ...
        if target_senders.is_empty() {
            warn!("No notification senders available for request: {}", request.title);
        }
    ...
    async fn send_with_retry(&self, sender: &dyn NotificationSender, request: &NotificationRequest) -> NotificationResult {
        ...
        while retry_count <= self.max_retries {
            if retry_count > 0 {
                ...
                debug!("Retrying notification '{}' via {} (attempt {})", request.title, sender.name(), retry_count);
            }

            match sender.send(&request.title, &request.message, &request.metadata).await {
                Ok(_) => {
                    info!("Successfully sent notification '{}' via {}", request.title, sender.name());
                ...
                Err(e) => {
                    ...
                    error!("Failed to send notification '{}' via {}: {}", request.title, sender.name(), e);
                    retry_count += 1;
                }
            }
        }
