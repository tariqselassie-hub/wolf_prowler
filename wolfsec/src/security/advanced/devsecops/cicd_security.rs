use crate::security::advanced::devsecops::{
    DevSecOpsConfig, FindingLocation, FindingSeverity, FindingStatus, FindingType, SecurityFinding,
    CICDPipeline, SecuredPipeline
};
use anyhow::Result;
use chrono::Utc;
use regex::Regex;
...
impl CICDSecurityManager {
    pub fn new(config: DevSecOpsConfig) -> Result<Self> {
        ...
        let secret_patterns = vec![
            ...
            ("AWS Secret Key".to_string(), Regex::new(r"(?i)aws_secret_access_key\s*=\s*[A-Za-z0-9/+=]{40}").unwrap()),
            ("Private Key".to_string(), Regex::new(r"BEGIN RSA PRIVATE KEY").unwrap()),
            ("Generic API Key".to_string(), Regex::new(r#"(?i)api[_-]?key\s*[:=]\s*['"][a-zA-Z0-9_\-]{16,}['"]"#).unwrap()),
            ("Generic Password".to_string(), Regex::new(r#"(?i)password\s*[:=]\s*['"][a-zA-Z0-9@#$%^&*()_\-+]{8,}['"]"#).unwrap()),
        ];

        let misconfig_patterns = vec![
            ("Privileged Mode".to_string(), Regex::new(r"(?i)privileged\s*[:=]\s*true").unwrap()),
            ("Docker Privileged Flag".to_string(), Regex::new(r"--privileged").unwrap()),
            ("Root User".to_string(), Regex::new(r"(?i)user\s+root").unwrap()),
            ("Root Login Permit".to_string(), Regex::new(r"(?i)PermitRootLogin\s+yes").unwrap()),
            ("Debug Mode".to_string(), Regex::new(r"(?i)debug\s*[:=]\s*true").unwrap()),
        ];
