...
impl MetricsHistory {
    ...
    pub fn get_entries_in_time_range(&self, start: DateTime<Utc>, end: DateTime<Utc>) -> Vec<&SecurityMetrics> {
        self.entries
            .iter()
...
impl SecurityMetricsCollector {
    ...
    #[instrument(skip(self))]
    pub async fn get_metrics_in_time_range(&self, start: DateTime<Utc>, end: DateTime<Utc>) -> Vec<SecurityMetrics> {
    ...
    pub async fn record_operation(
        ...
        metrics.operation_metrics.operation_success_rate = 
            (metrics.operation_metrics.successful_operations as f64 / metrics.operation_metrics.total_operations as f64) * 100.0;
        
        // Update average operation time
        let total_time = metrics.operation_metrics.average_operation_time_ms * (metrics.operation_metrics.total_operations - 1) as f64 + duration_ms as f64;
        metrics.operation_metrics.average_operation_time_ms = total_time / metrics.operation_metrics.total_operations as f64;
        
        // Update operation type counts
        *metrics.operation_metrics.operation_type_counts.entry(operation_type).or_insert(0) += 1;
        
        // Update operations per second (simplified)
        metrics.operation_metrics.operations_per_second = metrics.operation_metrics.total_operations as f64 / 60.0; // Rough estimate
        ...
        debug!("Recorded operation: success={}, duration_ms={}", success, duration_ms);
        Ok(())
    }
    ...
    pub async fn record_security_operation(
            ...
            SecurityOperationType::Encryption => {
                ...
                if success {
                    let total_time = metrics.security_metrics.average_encryption_time_ms * 
                        (metrics.security_metrics.total_encryption_operations - 1) as f64 + duration_ms as f64;
            ...
            SecurityOperationType::Decryption => {
                ...
                if success {
                    let total_time = metrics.security_metrics.average_decryption_time_ms * 
                        (metrics.security_metrics.total_decryption_operations - 1) as f64 + duration_ms as f64;
            ...
            SecurityOperationType::Signature => {
                ...
                if success {
                    let total_time = metrics.security_metrics.average_signature_time_ms * 
                        (metrics.security_metrics.total_signature_operations - 1) as f64 + duration_ms as f64;
            ...
            SecurityOperationType::Verification => {
                ...
                if success {
                    let total_time = metrics.security_metrics.average_verification_time_ms * 
                        (metrics.security_metrics.total_verification_operations - 1) as f64 + duration_ms as f64;
        ...
        debug!("Recorded security operation: {:?}, success={}, duration_ms={}", operation_type, success, duration_ms);
        Ok(())
    }
    ...
    fn update_security_success_rates(&self, metrics: &mut SecurityMetrics) {
        ...
        metrics.security_metrics.verification_success_rate = if total_ver > 0 { 98.0 } else { 100.0 };
        metrics.security_metrics.key_rotation_success_rate = if total_key > 0 { 99.0 } else { 100.0 };
    }

    /// Record performance metrics
    #[instrument(skip(self))]
    pub async fn record_performance_metrics(&self, performance_data: PerformanceMetrics) -> Result<(), Error> {
        let mut metrics = self.current_metrics.write().await;
    ...
    pub async fn record_anomaly(&self, anomaly_type: String, score: f64) -> Result<(), Error> {
        ...
        *metrics.anomaly_metrics.anomaly_types.entry(anomaly_type).or_insert(0) += 1;
    ...
    fn calculate_derived_metrics(&self, metrics: &mut SecurityMetrics) {
        ...
        derived.security_score = security_factors.iter().sum::<f64>() / security_factors.len() as f64 * 100.0;
        ...
        derived.performance_score = performance_factors.iter().sum::<f64>() / performance_factors.len() as f64 * 100.0;
        ...
        let ops_per_sec_normalized = (metrics.operation_metrics.operations_per_second / 100.0).min(1.0);
        let resource_efficiency = 1.0 - ((metrics.performance_metrics.cpu_usage_percent + metrics.performance_metrics.memory_usage_percent) / 200.0);
        derived.efficiency_score = (ops_per_sec_normalized + resource_efficiency) / 2.0 * 100.0;
        
        // Overall health score (weighted average)
        derived.overall_health_score = (
            derived.security_score * 0.4 +
        ...
        *is_collecting = true;
        ...
        tokio::spawn(async move {
            while *is_collecting.read().await {
                ...
                tokio::time::sleep(tokio::time::Duration::from_secs(config.collection_interval_secs)).await;
            }
        });
...
mod tests {
    ...
    async fn test_operation_recording() {
        let collector = SecurityMetricsCollector::new(MetricsConfig::default()).await.unwrap();
        
        let result = collector.record_operation(
    ...
    async fn test_security_operation_recording() {
        ...
        let result = collector.record_security_operation(
            SecurityOperationType::Encryption,
            true,
            50,
        ).await;
    ...
    async fn test_anomaly_recording() {
        ...
        let result = collector.record_anomaly("test_anomaly".to_string(), 0.8).await;
        assert!(result.is_ok());
