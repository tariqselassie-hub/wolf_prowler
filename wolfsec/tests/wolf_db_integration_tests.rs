...
use wolfsec::{
    ...
    domain::{
        entities::{Alert, AlertSeverity, AlertCategory},
...
async fn test_wolf_db_connection_and_initialization() {
    ...
    assert!(wolf_sec_result.is_ok(), "Failed to connect to WolfDb: {:?}", wolf_sec_result.err());
    ...
    assert!(storage.get_active_sk().is_some(), "Database should be unlocked with active keys");
}
...
async fn test_wolf_db_crud_operations() {
    ...
    let wolf_sec = WolfSecurity::create(config).await.expect("Failed to init WolfSecurity");
    ...
    assert!(save_result.is_ok(), "Failed to save alert: {:?}", save_result.err());
